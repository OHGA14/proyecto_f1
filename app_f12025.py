...
def _get_team_column(laps_df):
    for col in ("Team", "TeamName", "Constructor", "ConstructorName"):
        if col in laps_df.columns:
            return col
    if 'team_col_warned' not in st.session_state:
        st.info(f"No se encontró columna de equipo en laps. Columnas disponibles: {list(laps_df.columns)}")
        st.session_state['team_col_warned'] = True
    return None
...
_SELECTION_COLORS = build_driver_colors(selected_abbr, laps)
